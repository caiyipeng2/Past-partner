"""Bounded, manifest-driven parsing for WeChat backup ZIP packages."""
...
_SUPPORTED_FILE_TYPES = frozenset(
    {"generic_json", "generic_jsonl", "wechat_text", "wechat_html"}
)


class WeChatBackupError(ValueError):
...
class WeChatBackupParser:
    source_type = "wechat_backup"
    ...
    def probe(self, source: "ParserSource") -> "ParserProbe":
        ...
        if not source.path.is_file():
            return ParserProbe(self.source_type, 0.0, False, "微信备份包必须是 ZIP 文件")
    ...
    def validate(self, source: "ParserSource") -> "ParserValidation":
        ...
        if not source.path.is_file():
            return ParserValidation(False, "source_not_file", "微信备份包必须是 ZIP 文件")
        try:
            _inspect_archive(source.path)
        except WeChatBackupError as exc:
    ...
    def stream_records(self, source: "ParserSource") -> Iterator[NormalizedMessage]:
        ...
        with tempfile.TemporaryDirectory(prefix="past-partner-wechat-backup-") as temporary:
            ...
            try:
                ...
                for archive_file in plan.files:
                    ...
                    result = registry.parse(
                        extracted,
                        {"source_type": archive_file.source_type},
                    )
                    yield from result.records
            except WeChatBackupError:
                raise
            except (OSError, KeyError, RuntimeError, zipfile.BadZipFile) as exc:
                raise WeChatBackupError(
                    "corrupt_archive",
                    "微信备份包无法安全读取，可能已损坏",
                ) from exc
...
def _inspect_archive(path: Path) -> _ArchivePlan:
    try:
        with zipfile.ZipFile(path, "r") as archive:
            ...
            if not infos:
                raise WeChatBackupError("unsupported_manifest", "微信备份包不能为空")
            ...
            for info in infos:
                ...
                if normalized_name in by_name:
                    raise WeChatBackupError("duplicate_entry", "微信备份包包含重复条目")
                by_name[normalized_name] = info
                if info.is_dir():
                    continue
                mode = (info.external_attr >> 16) & 0xFFFF
                if stat.S_ISLNK(mode):
                    raise WeChatBackupError("unsupported_entry", "微信备份包不支持符号链接条目")
                if info.filename.casefold().endswith(".zip"):
                    raise WeChatBackupError("nested_archive", "微信备份包不支持嵌套 ZIP")
            ...
            if manifest_info is None:
                raise WeChatBackupError("unsupported_manifest", "微信备份包缺少根目录 manifest.json")
            if manifest_info.file_size > MAX_MANIFEST_BYTES:
                raise WeChatBackupError("archive_limits_exceeded", "微信 manifest 超过安全大小上限")
            try:
                manifest = json.loads(archive.read(manifest_info).decode("utf-8"))
            except (UnicodeDecodeError, json.JSONDecodeError, OSError, zipfile.BadZipFile) as exc:
                raise WeChatBackupError("unsupported_manifest", "微信 manifest 不是有效 UTF-8 JSON") from exc
            if not isinstance(manifest, dict):
                raise WeChatBackupError("unsupported_manifest", "微信 manifest 必须是 JSON 对象")
            if manifest.get("schema_version") != 1:
                raise WeChatBackupError("unsupported_manifest", "微信 manifest schema_version 不受支持")
            platform = manifest.get("platform", manifest.get("source_type"))
            if not isinstance(platform, str) or platform.casefold() not in {"wechat", "weixin", "wechat_backup"}:
                raise WeChatBackupError("unsupported_manifest", "微信 manifest 平台标识不受支持")

            raw_records = manifest.get("records")
            raw_files = manifest.get("files")
            if raw_records is not None and raw_files is not None:
                raise WeChatBackupError("unsupported_manifest", "微信 manifest 不能同时包含 records 和 files")
            if raw_records is not None:
                if not isinstance(raw_records, list) or not all(isinstance(item, dict) for item in raw_records):
                    raise WeChatBackupError("unsupported_manifest", "微信 manifest records 格式不受支持")
                _reject_unlisted_files(by_name, manifest_info.filename)
                return _ArchivePlan(manifest, (), tuple(dict(item) for item in raw_records))
            if not isinstance(raw_files, list) or not raw_files:
                raise WeChatBackupError("unsupported_manifest", "微信 manifest 必须声明 files 或 records")
            if len(raw_files) > MAX_MANIFEST_FILES:
                raise WeChatBackupError("archive_limits_exceeded", "微信 manifest 文件数量超过安全上限")
            ...
            return _ArchivePlan(manifest, files, ())
    except WeChatBackupError:
        raise
    except (OSError, zipfile.BadZipFile, ValueError) as exc:
        raise WeChatBackupError("corrupt_archive", "微信备份包无法读取，可能已损坏") from exc
...
def _extract_files(path: Path, files: Sequence[_ArchiveFile], destination_root: Path) -> None:
    ...
    try:
        with zipfile.ZipFile(path, "r") as archive:
            for archive_file in files:
                ...
                try:
                    ...
                    raise WeChatBackupError("path_traversal", "微信备份包条目超出安全目录") from exc
        ...
        raise WeChatBackupError("corrupt_archive", "微信备份包无法安全读取，可能已损坏") from exc
...
def _normalize_file(
    ...
    if not isinstance(value, dict):
        raise WeChatBackupError("unsupported_manifest", "微信 manifest files 项格式不受支持")
    raw_name = value.get("path")
    if not isinstance(raw_name, str):
        raise WeChatBackupError("unsupported_manifest", "微信 manifest 文件路径缺失")
    _validate_member_name(raw_name)
    if raw_name.casefold() == manifest_name.casefold():
        raise WeChatBackupError("unsupported_manifest", "微信 manifest 不能把自身作为消息文件")
    info = by_name.get(raw_name.casefold())
    if info is None or info.is_dir():
        raise WeChatBackupError("unsupported_manifest", "微信 manifest 引用了不存在的文件")
...
def _normalize_source_type(raw_type: object, name: str) -> str:
    ...
    if source_type not in _SUPPORTED_FILE_TYPES:
        raise WeChatBackupError("unsupported_manifest", "微信 manifest 声明了不受支持的消息文件格式")
    return source_type


def _manifest_info(by_name: dict[str, zipfile.ZipInfo]) -> zipfile.ZipInfo | None:
    for name in ("manifest.json", "wechat-manifest.json"):
...
def _reject_unlisted_files(
    ...
    for name, info in by_name.items():
        ...
        if name not in listed_names:
            raise WeChatBackupError("unsupported_manifest", "微信 manifest 未覆盖备份包中的全部文件")


def _validate_member_name(name: str) -> None:
    if not name or "\x00" in name or "\\" in name:
        raise WeChatBackupError("path_traversal", "微信备份包包含不安全的文件路径")
...
if __name__ == "__main__":
    raise SystemExit("wechat_backup is a parser module, not a command-line entry point")
