...
class Application:
    ...
    def from_config(cls, config: ServerConfig) -> "Application":
        ...
        media_capabilities = {
            provider_id: adapter.config.media_capabilities
            for provider_id, adapter in adapters.items()
            if hasattr(adapter, "config") and getattr(adapter.config, "media_capabilities", None)
        }
        catalog = catalog.with_configured(
            set(adapters),
            runtime_models,
            fine_tuning_models,
