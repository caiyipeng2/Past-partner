...
P0-05 提供版本化 AES-256-GCM 信封加密服务；P0-06 已将上传分片和合并对象接入该服务。每个分片使用独立随机数据密钥和 nonce，AAD 绑定导入 ID、分片序号及结束标记，合并对象追加认证结束哨兵。单次加密默认限制为 64 MiB，3 GiB 导入始终按有界分片处理。当前人物资料、导入清单等 JSON 元数据仍未加密，字段级加密属于后续任务，具体限制见 `docs/privacy_policy.md`。
