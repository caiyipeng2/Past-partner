...
class SQLiteMigrationTests(unittest.TestCase):
    ...
    def test_creates_a_versioned_database_from_an_empty_directory(self) -> None:
        ...
        with closing(sqlite3.connect(self.database_path)) as connection:
            tables = connection.execute(
                ...
                "AND name IN ('personas', 'training_jobs', 'style_profiles', 'long_term_memories', 'vector_indexes', 'task_broker_outbox') ORDER BY name"
            ).fetchall()
        self.assertEqual(
            [
                ("long_term_memories",),
    ...
    def test_upgrades_a_version_one_database_to_persona_repository(self) -> None:
        ...
        self.assertEqual(17, SQLiteMigrator(self.database_path).migrate())
        with closing(sqlite3.connect(self.database_path)) as connection:
    ...
    def test_concurrent_migration_is_idempotent(self) -> None:
        ...
        self.assertEqual([17, 17], versions)
        with closing(sqlite3.connect(self.database_path)) as connection:
            count = connection.execute("SELECT COUNT(*) FROM schema_migrations").fetchone()[0]
        self.assertEqual(17, count)
