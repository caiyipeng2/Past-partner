...
class PrivacyPolicyContractTests(unittest.TestCase):
    ...
    def test_policy_discloses_current_data_flow_and_limits(self) -> None:
        required_disclosures = (
            ...
            "当前版本保留本地 owner 的 Bearer 会话，同时在 development/test 模式提供受限的本地账户主体",
            "不同本地账户无法读取或删除彼此资源",
            "同租户 `admin` 可查看成员的有限身份元数据",
            "POST /api/v1/tenant/members/{user_id}/revoke-sessions",
            "PATCH /api/v1/tenant/members/{user_id}",
            "调整成员角色或停用/恢复成员",
            "PATCH /api/v1/tenant/members/{user_id}/status",
            "停用会撤销该成员的访问会话和 OIDC refresh token",
            "不提供本地注册、密码找回、租户创建、账务管理或上游 IdP 注销",
            "R1-02 已提供成功标准化导入的可配置保留期清理、原始载荷完整归档导出和当前本地 owner 的级联删除",
