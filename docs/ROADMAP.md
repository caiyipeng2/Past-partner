...
| R2-02 | OCR、ASR、视觉/音视频语义处理和媒体模型实际调用 | P1-10/P2-06 授权门控、R0-03 Provider | 当前已完成授权后图像分析、显式 OpenAI-compatible 音频转写和配置端点的视频语义分析；OCR 专项适配仍待完成 |
...
- 默认真实 Provider 仍主要声明文本 chat；R2-02 已为 OpenAI-compatible 适配器增加受目录 vision 能力约束的图像分析、通过 `*_AUDIO_MODELS` 显式开启的 `/audio/transcriptions` 音频转写，以及通过 `*_VIDEO_MODELS` + `*_VIDEO_ENDPOINT_PATH` 显式开启的 Provider 专用视频语义分析，OCR 专项、流式和 Embedding 仍未完成；R0-04 仅为显式开启的千问模型增加原生微调能力。
