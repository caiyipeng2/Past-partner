...
7. **通用 HTML 聊天记录** (.html、.htm 或其他扩展名)
   - 支持 `message`、`chat-item`、`chat-entry`、`record`、`utterance` 等常见消息容器及 `data-*` 属性
   - 支持发送者、时间、正文子节点、HTML 实体、`<br>` 换行和 UTF-8、UTF-16、GB18030 编码
   - 脚本、样式和模板内容不会作为消息导入；没有可识别消息容器会返回 `unsupported_format`

8. **QQ 文本和 HTML 导出** (.txt, .html, .htm)
   - 支持常见的“时间 + 发送者 + 多行正文”文本导出
   - 支持带消息容器、发送者、时间和正文标记的 HTML 导出
   - 支持 UTF-8、UTF-16 和 GB18030 编码，解析结果的 `source_type` 为 `qq_text` 或 `qq_html`

9. **通用 SQLite 数据库目录** (包含 `.db` 文件的用户选择目录)
   - 支持消息表名和发送者、内容、时间、消息类型等常见字段别名的 schema 自动探测
   - 解析前复制 `.db` 及现有 `-wal`/`-shm` 文件形成一致只读快照，源目录不会被写入
   - 单个 `.db`、非 SQLite/加密库、未知 schema 和空消息目录会返回明确错误

10. **微信数据库目录** (`db_storage/`、`Msg/`)
...
11. **其他常见资料**
   ...
   - P0-18/P0-20/P0-21 已提供 TXT、JSON、JSONL 的统一消息标准化核心；P0-25 增加微信 TXT/HTML 导出解析；P0-26 增加 QQ TXT/HTML 导出解析；P0-27 增加默认关闭的终态原始导入保留期清理；P0-28 提供 owner 级人物、导入任务、参与者映射、预览修正和加密清单元数据导出；P0-29 提供 owner 级人物删除级联清理；P0-30/P0-31 提供微信和 QQ 明文 SQLite 数据库目录解析；P0-32/P0-33 提供微信和 QQ ZIP 备份包安全解析；P1-01/P1-02/P1-03 提供 CSV、XML 和通用 HTML 解析。按原始任务表，P1-04 ZIP 安全边界由备份解析覆盖，P1-05/P1-06/P1-07 分别对应通用 SQLite、微信原生数据库和 QQ 原生数据库；当前 QQ 解析支持 `.db`、`.sqlite`、`.sqlite3` 及 WAL/SHM sidecar。私有加密备份仍只返回明确错误，不猜测密钥。

说明：P1-03 新增通用 HTML 解析；平台标记明确的微信/QQ HTML 仍优先使用对应平台解析器，通用解析器只作为跨平台或无扩展名 HTML 的回退。按原始任务表，P1-04 的 ZIP 安全边界由微信/QQ 备份解析覆盖，P1-05 是通用 SQLite schema 探测，P1-06/P1-07 是微信/QQ 原生数据库。P1-08 已独立收口 DOCX 对话文本解析，包含段落内换行/制表符保留和安全归档校验；P1-09 现已独立收口 PDF 对话文本解析，包含可选 `pypdf`、有界文本回退、UTF-16 十六进制文本和加密/页数上限校验；P1-10 已提供按供应商、模型、数据类别和授权范围记录的媒体第三方处理授权、查询与撤回接口。P2-01 已提供模型目录能力、上下文长度、隐私元数据、价格刷新时间和成本估算接口；价格通过 `PAST_PARTNER_MODEL_PRICING_JSON` 配置，未配置价格不会生成估算。P2-02 已接入 OpenAI、Anthropic、Gemini、DeepSeek、小米 MiMo、阿里千问、Ollama 和自定义 OpenAI-compatible 的文本聊天适配器；适配器只在服务端凭据和模型允许列表有效时启用。P2-03 已增加仅基于人物文本的风格画像统计；P2-04 已增加事实、事件、关系、偏好和时间线的本地长期记忆候选，默认待审核并保留有限证据引用，不调用模型或上传原始内容。R1-01 已将画像、长期记忆和版本化稀疏索引按 owner/persona 加密持久化，并提供人物范围 API；浏览器端目录聚合、文档内媒体内容分析和实际第三方媒体调用仍属于后续任务，本地上传不等于第三方处理授权。
...
2. **隐私保护**: 原始媒体发送给第三方模型前必须取得按人物、供应商、模型、数据类别和用途划分的明确授权；撤回授权只阻止未来处理，不代表第三方已接收数据会自动删除
3. **数据安全**: 当前本地开发版写入配置的数据目录，并通过回环地址初始化本地 owner Bearer 会话；单个导入和人物级导入数据可通过 API 删除，人物、导入任务和清单元数据可通过 API 导出。设置 `PAST_PARTNER_RAW_RETENTION_SECONDS` 后，服务启动会清理当前 owner 名下过期的 `failed` 或 `cancelled` 导入及其加密对象；设置 `PAST_PARTNER_NORMALIZED_RETENTION_SECONDS` 后会清理已成功标准化且超过保留期的导入，两个设置默认关闭且受五年硬上限约束。`GET /api/v1/data-export/archive` 提供包含原始 payload 的流式 ZIP 归档，manifest 声明原始对象数量和字节数；`POST /api/v1/data-deletion` 提供当前单 owner 的确认式级联删除和匿名回执。OIDC 多用户账户、正式租户管理和第三方 provider 侧删除仍属于后续生产阶段，不能把本地删除回执理解为第三方数据已删除
