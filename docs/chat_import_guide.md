...
说明：P1-03 新增通用 HTML 解析；平台标记明确的微信/QQ HTML 仍优先使用对应平台解析器，通用解析器只作为跨平台或无扩展名 HTML 的回退。P1-04 的通用 SQLite 解析入口用于用户主动选择的数据库目录；P1-05 会从 JSON/JSONL、CSV、XML、HTML 和通用 SQLite 消息中提取附件引用元数据。解析器只保留相对路径或 HTTPS/HTTP URL、文件名、MIME、类型、大小和可选校验值，不读取媒体内容，不接受绝对路径、路径穿越或内嵌原始字节。浏览器端目录聚合仍属于后续任务。
