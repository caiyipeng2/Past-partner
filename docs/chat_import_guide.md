...
说明：P1-03 新增通用 HTML 解析；平台标记明确的微信/QQ HTML 仍优先使用对应平台解析器，通用解析器只作为跨平台或无扩展名 HTML 的回退。P1-04 的通用 SQLite 解析入口用于用户主动选择的数据库目录；P1-05 会从 JSON/JSONL、CSV、XML、HTML 和通用 SQLite 消息中提取附件引用元数据；P1-06 增加 DOCX/PDF 文档中的对话文本解析。DOCX 只读取安全包内的主文档 XML，PDF 优先使用可选 `pypdf`，并对损坏、加密、超限和无对话文本返回明确错误。浏览器端目录聚合、文档内媒体内容分析仍属于后续任务。
