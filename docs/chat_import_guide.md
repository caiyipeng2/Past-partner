...
说明：P1-03 新增通用 HTML 解析；平台标记明确的微信/QQ HTML 仍优先使用对应平台解析器，通用解析器只作为跨平台或无扩展名 HTML 的回退。按原始任务表，P1-04 的 ZIP 安全边界由微信/QQ 备份解析覆盖，P1-05 是通用 SQLite schema 探测，P1-06/P1-07 是微信/QQ 原生数据库。附件元数据和 DOCX/PDF 文档解析属于已提前落地的补充实现，分别覆盖原 P1-08/P1-09 相关能力。浏览器端目录聚合、文档内媒体内容分析仍属于后续任务。
