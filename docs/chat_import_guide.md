...
说明：P1-03 新增通用 HTML 解析；平台标记明确的微信/QQ HTML 仍优先使用对应平台解析器，通用解析器只作为跨平台或无扩展名 HTML 的回退。按原始任务表，P1-04 的 ZIP 安全边界由微信/QQ 备份解析覆盖，P1-05 是通用 SQLite schema 探测，P1-06/P1-07 是微信/QQ 原生数据库。P1-08 现已独立收口 DOCX 对话文本解析，包含段落内换行/制表符保留和安全归档校验；PDF 属于原 P1-09，虽有提前落地的兼容实现，仍按原编号单独核对。浏览器端目录聚合、文档内媒体内容分析仍属于后续任务。
